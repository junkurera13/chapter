...
export function describeMemorySubmissionFailure(
  ...
  if (code === "IMAGE_REFERENCE_INVALID") {
    return {
      message: serverMessage || "One of those photos needs to be uploaded again.",
  ...
  if (code === "MEMORY_INPUT_INVALID" && serverMessage) {
    return { message: serverMessage, requiresAuthentication: false };
  }
  if (status === 413) {
    return {
...
export async function uploadMemoryPhoto(
  ...
  const payload = (await response.json().catch(() => ({}))) as {
    value?: Omit<UploadedMemoryPhoto, "context">;
    error?: string;
  };
  if (!response.ok || !payload.value) {
    throw {
      status: response.status,
      data: { error: payload.error, code: "IMAGE_REFERENCE_INVALID" },
    };
  }
  return { ...payload.value, context: context.trim() };
}
...
export async function createExperienceMemory({
  ...
  try {
    ...
    if (reachedTimeout) {
      throw {
        status: 504,
        data: { code: "MEMORY_TIMEOUT", error: "Chapter took too long." },
      };
    }
    throw error;
  } finally {
  ...
  if (!response.ok || !payload.value) {
    throw {
      status: response.status,
      data: { error: payload.error, code: payload.code },
    };
  }
